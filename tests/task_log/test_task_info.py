...
def test_name_lowercase_conversion():
    task = Task(
        Id="task-123", ProjectId="proj-456", Name="Sample-Task", Description="Test"
    )
    assert task.Name == "sample-task"
...
def test_display_as_list(capsys):
    tasks = [
        Task(
            Id="task-1", ProjectId="proj-1", Name="task-one", Description="First task"
        ),
        Task(
            Id="task-2", ProjectId="proj-1", Name="task-two", Description="Second task"
        ),
    ]
