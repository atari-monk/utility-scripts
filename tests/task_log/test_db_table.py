...
class TestDbTable(unittest.TestCase):
    def setUp(self):
        ...
        self.db_info = DbPath(self.test_dir, "test_db")
    ...
    def test_get_path_with_string_naming_logic(self):
        table_name = "projects"
        db_table = DbTablePath(self.db_info, lambda: table_name)
        
        expected_path = self.db_info.path / f"{table_name}.json"
        self.assertEqual(db_table.get_path(), expected_path)
        
        expected_path = self.db_info.path / f"{table_name}.csv"
    ...
    def test_get_path_with_callable_naming_logic(self):
        ...
        db_table = DbTablePath(self.db_info, naming_func)
        
        expected_path = self.db_info.path / "dynamic_name.json"
        self.assertEqual(db_table.get_path(), expected_path)
